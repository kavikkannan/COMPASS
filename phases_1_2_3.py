...
This page is part of a legal tech platform where the assistant chatbot (shown in the image) serves as the primary interface,
connecting users to all other pages and features through an intelligent agentic flow system.
...
Consider how well this page supports the assistant's ability to guide users to the right features and workflows.
...
Feedback Questionnaire (fill out completely and be specific):
...
3. **Open Feedback:** [**CRITICAL: Provide a direct quote from the persona's point of view detailing their experience.** For example: "I was trying to create a new case file, but I couldn't find the 'New Case' button. I expected it to be at the top right, but it was hidden in a menu."]

**Part B: Specific UI & Design Details**
1. **Readability:** [**Be specific.** Example: "The body text font size is fine, but the labels under the input fields are too small and light grey, making them hard to read."]
2. **Layout & Spacing:** [**Be specific.** Example: "The buttons are too close together - I almost clicked 'Cancel' when I meant to click 'Save'. There should be more space between them."]
3. **Color & Visuals:** [**Be specific.** Example: "The red error message blends into the background too much. It needs to be brighter or have a different background color to stand out."]
4. **Overall Impression:** [**Be specific.** Example: "The page looks cluttered because there are too many elements competing for attention. The search bar, filters, and action buttons are all the same size and color."]

**Part C: Feature Evaluation & Gaps**
1. **Feature Usefulness:** [**Be specific.** Example: "The 'Quick Search' feature was helpful because I could find my case quickly, but the 'Advanced Filters' section was confusing - I couldn't figure out how to filter by date range."]
2. **Missing Features:** [**Name the specific feature you were looking for.** Example: "I really wish there was a 'Bulk Upload' button on this page so I could add all 20 evidence documents at once instead of one by one."]

**Part D: Assistant Integration & Agentic Flow**
1. **Assistant Guidance:** [**How well does the assistant help you navigate to the right features?** Example: "The assistant suggested I use the 'Document Analysis' workflow, but I couldn't figure out how to access it from this page. I expected a direct link or button."]
2. **Workflow Discovery:** [**Can you easily discover what workflows are available through the assistant?** Example: "I asked the assistant about patent searches, but it only showed me general legal workflows. I need to know if there are specialized patent tools available."]
3. **Cross-Page Navigation:** [**How well does this page connect to other parts of the system?** Example: "I can see the assistant mentioned 'Case Management' but there's no clear way to get there from this page. I expected a navigation menu or direct links."]

**IMPORTANT: Write your responses as if you are the persona speaking directly. Use first person ("I think...", "I expected...", "I couldn't find..."). Be specific about what you see in the image and what you expected to see. Avoid generic statements like "the layout could be improved" - instead say exactly what's wrong and what you expected.**
"""

CONSOLIDATION_PROMPT_TEMPLATE = """
Analyze the complete set of user feedback questionnaires provided below. Consolidate the findings
into a single markdown table with the following columns: 'Page', 'Persona', 'Feedback Type (UI, UX Flow, Feature Request, Assistant Integration)',
and 'Specific Feedback/Quote'.

For each feedback entry, extract the most specific and actionable quotes from the persona's responses.
Focus on concrete details like specific UI elements, exact quotes about user confusion, precise feature requests,
and feedback about how well the assistant integrates with the page and guides users through the agentic flow.
...
def run_phases_1_2_3(pages_dir: str):
    """Run phases 1, 2, and 3 of the design survey simulation."""
    create_dirs()
        ...
        for page_name, inventory in page_inventories.items():
            ...
            for task_num, task in enumerate(tasks):
                ...
                if os.path.exists(feedback_path):
                    print(f"    - Skipping Task '{task[:40]}...' on Page '{page_name}' (file already exists)")
                    # Still read the existing feedback to include in master collection
                    try:
                        with open(feedback_path, "r", encoding="utf-8") as f:
                            existing_feedback = f.read()
                        all_feedback_text += f"\n\n--- FEEDBACK FROM {persona_name} ON {page_name} ---\n\n{existing_feedback}"
                    except Exception as e:
                        print(f"    - Warning: Could not read existing feedback file {feedback_filename}: {e}")
                    continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run phases 1, 2, and 3 of the LLM-powered design survey.",
    ...
    run_phases_1_2_3(args.pages_dir)
